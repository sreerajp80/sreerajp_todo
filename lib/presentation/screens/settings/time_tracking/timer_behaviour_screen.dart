...
class TimerBehaviourScreen extends ConsumerWidget {
  ...
  /// The keep-awake and notification channels are Android only, so the
  /// switches are hidden rather than shown doing nothing on other platforms.
  bool get _supportsAndroidFeatures => !kIsWeb && Platform.isAndroid;
              ...
              child: Column(
                children: [
                  ...
                  if (_supportsAndroidFeatures) ...[
                    ...
                    SwitchListTile(
                      contentPadding: EdgeInsets.zero,
                      value: settings.showRunningNotification,
                      onChanged: (enabled) async {
                        if (enabled) {
                          final granted = await ref
                              .read(runningNotificationChannelProvider)
                              .requestPermission();
                          if (!granted) return;
                        }
                        await notifier.setShowRunningNotification(enabled);
                      },
                      title: Text(l10n.trackingRunningNotification),
                      subtitle: Text(l10n.trackingRunningNotificationDetail),
                    ),
                  ],
                ],
              ),
