...
class FakeSpacedRepetitionItemEntity extends Fake
    implements SpacedRepetitionItemEntity {}
...
void main() {
  ...
  test('Hard confidence resets interval to 1 day and level to 1', () async {
    ...
    when(
      () => segmentRepo.getRunningSegment('t-1'),
    ).thenAnswer((_) async => null);
    when(
      () => todoRepo.updateStatus('t-1', TodoStatus.completed),
    ).thenAnswer((_) async {});
    ...
    final captured =
        verify(() => srsRepo.updateItem(captureAny())).captured.single
            as SpacedRepetitionItemEntity;
    expect(captured.level, 1);
    expect(captured.intervalDays, 1);
  });

  test(
    'Easy confidence increases level and expands interval to 7 * level',
    () async {
      final now = DateTime.now().toUtc().toIso8601String();
      final todo = TodoEntity(
        id: 't-1',
