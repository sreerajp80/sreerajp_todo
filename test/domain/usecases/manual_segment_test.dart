...
void main() {
  ...
  test(
    'inserts manual segment with manual=true and computed duration',
    () async {
  ...
  test('rejects overlapping manual segment', () async {
    ...
    await segmentRepo.insertManualSegment(
      makeManualSegment(id: 'man-2a', todoId: 'ms-2', start: start1, end: end1),
    );

    final start2 = DateTime(2026, 3, 21, 9, 30);
    final end2 = DateTime(2026, 3, 21, 10, 30);
    expect(
      () => segmentRepo.insertManualSegment(
        makeManualSegment(
          id: 'man-2b',
          todoId: 'ms-2',
          start: start2,
  ...
  test('rejects manual segment on past-date todo', () async {
    ...
    expect(
      () => segmentRepo.insertManualSegment(
        makeManualSegment(id: 'man-3', todoId: 'ms-3', start: start, end: end),
      ),
  ...
  test('rejects manual segment on completed todo', () async {
    ...
    expect(
      () => segmentRepo.insertManualSegment(
        makeManualSegment(id: 'man-4', todoId: 'ms-4', start: start, end: end),
      ),
  ...
  test('allows non-overlapping manual segments', () async {
    await todoRepo.createTodo(makeTodo(id: 'ms-5', title: 'No overlap'));

    final start1 = DateTime(2026, 3, 21, 9, 0);
    final end1 = DateTime(2026, 3, 21, 10, 0);
    await segmentRepo.insertManualSegment(
      makeManualSegment(id: 'man-5a', todoId: 'ms-5', start: start1, end: end1),
    );

    final start2 = DateTime(2026, 3, 21, 10, 0);
    final end2 = DateTime(2026, 3, 21, 11, 0);
    await segmentRepo.insertManualSegment(
      makeManualSegment(id: 'man-5b', todoId: 'ms-5', start: start2, end: end2),
    );
